...
#define ADD_HR_R6_FLAGS_SRC  CPU,   CLEAR, ALU,   ALU
#define ADD_INCR16SP_FLAGS_SRC CPU , CPU , CPU ,  CPU
#define ROT_CARRY_FLAGS_SRC ALU ,  CLEAR , CLEAR , ALU
#define BIT_U3_R8_FLAGS_SRC ALU , CLEAR , SET , CPU
#define CHG_U3_R8_FLAGS_SRC CPU , CPU , CPU , CPU 
...
#define combine_flags_set_HL(cpu, ...) \
    do { \
        M_EXIT_IF_ERR(cpu_combine_alu_flags(cpu, __VA_ARGS__)); \
        cpu_HL_set(cpu,cpu->alu.value); \
    } while(0)
    
#define combine_flags_set_register(cpu,index, ...) \
    do { \
        M_EXIT_IF_ERR(cpu_combine_alu_flags(cpu, __VA_ARGS__)); \
        cpu_reg_set(cpu,extract_reg(lu->opcode,index),cpu->alu.value); \
    } while(0)

#define combine_flags_set_register_SP(cpu, ...) \
    do { \
        M_EXIT_IF_ERR(cpu_combine_alu_flags(cpu, __VA_ARGS__)); \
        cpu_reg_pair_SP_set(cpu,extract_reg(lu->opcode,4),cpu->alu.value); \
    } while(0)
