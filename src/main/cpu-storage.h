...
/**
 * @brief Reads data at BC address from bus
 */
#define cpu_read_at_BC(cpu) \
    cpu_read_at_idx(cpu, cpu_BC_get(cpu))

/**
 * @brief Reads data at DE address from bus
 */
#define cpu_read_at_DE(cpu) \
    cpu_read_at_idx(cpu, cpu_DE_get(cpu))
    
/**
 * @brief Reads data at REGISTERS_START( = 0xFF00) + C  address from bus
 */
#define cpu_read_at_Reg_Start_plus_C(cpu) \
    cpu_read_at_idx(cpu, REGISTERS_START + cpu_C_get(cpu))
...
#define cpu_write_at_Reg_Start_plus_C(cpu, data) \
    cpu_write_at_idx(cpu, REGISTERS_START +cpu_C_get(cpu), data)
